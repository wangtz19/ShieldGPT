...
PcapWriter::PcapWriter(const char *pcap_file_dir, uint32_t kCaplen, bool overwrite, int data_link_type){
    kCaplen_ = kCaplen;
    generate_template();
    pcap_descr_ = pcap_open_dead_with_tstamp_precision(data_link_type, kCaplen_, PCAP_TSTAMP_PRECISION_NANO);
    if (overwrite) {
