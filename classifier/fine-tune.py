...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        log_stats = {**{f'train_{k}': v if not isinstance(v, np.ndarray) else v.tolist() for k, v in train_stats.items()},
                     **{f'test_{k}': v if not isinstance(v, np.ndarray) else v.tolist() for k, v in test_stats.items()},
                     'epoch': epoch,
